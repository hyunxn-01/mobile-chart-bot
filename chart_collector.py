...
AXIS_PV = 'v12-brief5'   # 브리핑 프롬프트 버전(바꾸면 캐시 무효화·전 시장 1회 재생성). v12: 분석 개수 고정 해제(중요요소 빠짐없이), 핵심 시사점도 요약+분석. v11: 라벨 금지·키포인트 요약·개조식·자기참조/대시 금지
...
def _axis_prompt(scope_label, axis_label, digest, is_region, grounded=False, prior_note=''):
    ...
    return (f"다음은 App Store 게임 '매출' 차트의 '{scope_label}' {axis_label} 스냅샷·추이다.\n\n"
            ...
            "각 항목은 '## 제목' 한 줄로 시작하고, 그 아래 불릿(-)으로 쓴다. 첫 불릿: 그 항목의 핵심을 한 줄로 압축한 요약(막연한 주제문 금지, 실제 시장·장르·게임·수치를 콕 집어 개조식 키포인트로. 예: '한국 MMORPG · 북미 캐주얼 퍼즐·보드 · 일본 가챠 RPG·스포츠 시뮬'). 이후 불릿: 요약을 뒷받침하는 근거·세부를 중요한 것은 빠짐없이 담는다(보통 3개 안팎, 중요하면 그 이상도. 서로 다른 포인트, 중복 없이, 주요 분석 요소 누락 금지). '요약'·'분석' 같은 라벨 단어를 불릿 앞에 붙이지 말고 내용만 쓸 것. 요약만 훑어도 알맹이가 잡히고, 펼치면 중요한 분석이 다 담기게. 항목 제목은 그대로 둘 것.\n"
            "## 시장 구조\n## 주요 게임·퍼블리셔\n## 주요 순위 변동\n## 장르 기회\n## 핵심 시사점\n\n"
