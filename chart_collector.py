...
AXIS_PV = 'v12-brief5'   # 브리핑 프롬프트 버전(바꾸면 캐시 무효화·전 시장 1회 재생성). v12: 분석 개수 고정 해제(중요요소 빠짐없이), 핵심 시사점도 요약+분석. 2단계 생성 코드 반영(분석 먼저→요약, 요약⊆분석). 값 안 올림=강제 재생성 안 함, 다음 주간(월요일) 정기 갱신 때 자연 적용 · v11: 라벨 금지·키포인트 요약·개조식·자기참조/대시 금지 · 2026-07-09 요약1문장 강화+grounding→분석(값 유지=강제재생성 안 함, 렌더 분리로 기존 보정·다음 주간 갱신 때 자연 반영)
