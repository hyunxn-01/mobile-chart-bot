...
AXIS_PV = 'v11-brief4'   # 브리핑 프롬프트 버전(바꾸면 캐시 무효화·전 시장 1회 재생성). v11: 요약=키포인트 한 줄, 분석 3불릿, '요약:/분석:' 라벨 금지, 개조식, 자기참조·대시 금지, 섹션명 실무화
...
def _axis_prompt(scope_label, axis_label, digest, is_region, grounded=False, prior_note=''):
    ...
    _pb = (f"\n\n[직전 '{axis_label}' 분석 메모 — 저비중·연속성 검증용] {prior_note}\n"
           "→ 위 신호가 이번 데이터로도 유효한지 '## 주요 순위 변동'에 현재 사실로만 반영하라(맞으면 '~ 지속', 바뀌면 '~로 전환'). '지난 분석/저번' 자기참조 금지. "
    ...
    return (f"다음은 App Store 게임 '매출' 차트의 '{scope_label}' {axis_label} 스냅샷·추이다.\n\n"
            ...
            "각 항목은 '## 제목' 한 줄로 시작하고, 그 아래 불릿(-)으로 쓴다. 첫 불릿: 그 항목의 핵심을 한 줄로 압축한 요약(막연한 주제문 금지, 실제 시장·장르·게임·수치를 콕 집어 개조식 키포인트로. 예: '한국 MMORPG · 북미 캐주얼 퍼즐·보드 · 일본 가챠 RPG·스포츠 시뮬'). 이후 3개 불릿: 요약을 뒷받침하는 서로 다른 근거·세부(3개, 중복 없이). '요약'·'분석' 같은 라벨 단어를 불릿 앞에 붙이지 말고 내용만 쓸 것. 요약만 훑어도 알맹이가 잡히게. 항목 제목은 그대로 둘 것.\n"
            "## 시장 구조\n## 주요 게임·퍼블리셔\n## 주요 순위 변동\n## 장르 기회\n## 핵심 시사점\n\n"
            ...
            "굵게(**)는 게임명·장르·퍼블리셔·국가명에만. 이모지·구분선(---) 금지. 문장에 'ㅡ' 대시(em·en 대시 포함) 쓰지 말고 마침표·쉼표·괄호·콜론으로. 문장은 짧은 개조식(명사형 종결. 예: '좌우함'·'유리'). 모호한 조어 금지, 실무에서 쓰는 명확한 용어로. '지난 분석/저번 분석/앞서' 같은 자기참조 표현 금지. 브리핑은 그것만 봐도 이해되게 사실을 직접 서술(연속성은 '지속·유지·전환' 등 현재 사실로만). 한국어, 군더더기 없이. '게임/Games/游戏'는 분석 카테고리이지 장르가 아니다(장르는 구체 서브장르 MMORPG·4X·매치3 등으로만, '상위 100이 전부 게임' 류 자명한 서술 금지). 게임명 고유명사 외엔 한국어로만(游戏·ゲーム 등 외국어 일반어 금지)." + _pb + INDUSTRY_VOICE + (GROUNDING if grounded else ""))
...
def build_global_brief(collected):
    ...
    for axis_key, axis_label in BRIEF_AXES:
        ...
        prompt = (f"다음은 같은 기간 App Store 게임 매출 차트를, 주요 시장은 개별·중소규모는 지역으로 묶어 '{axis_label}'({win}) 시간축으로 분석한 결과다.\n\n{body}\n\n"
                  ...
                  "하위 분석에 없는 사실을 지어내지 말 것. 굵게(**)는 게임명·장르·퍼블리셔·국가/지역명에만. 이모지·구분선(---) 금지. 문장에 'ㅡ' 대시(em·en 대시 포함) 쓰지 말고 마침표·쉼표·괄호·콜론으로. 문장은 짧은 개조식(명사형 종결. 예: '좌우함'·'유리'). 모호한 조어 금지, 실무에서 쓰는 명확한 용어로. '지난 분석/저번 분석/앞서' 같은 자기참조 표현 금지. 브리핑은 그것만 봐도 이해되게 사실을 직접 서술(연속성은 '지속·유지·전환' 등 현재 사실로만). 한국어, 군더더기 없이. '게임/Games/游戏'는 분석 카테고리이지 장르가 아니다(장르는 구체 서브장르 MMORPG·4X·매치3 등으로만, '상위 100이 전부 게임' 류 자명한 서술 금지). 게임명 고유명사 외엔 한국어로만(游戏·ゲーム 등 외국어 일반어 금지)." + _gpb + INDUSTRY_VOICE + (GROUNDING if axis_key == 'weekly' else ""))
        try:
